...
console.log(
  "Mastering Ethereum - web3.js basic interactions using async/await"
);
...
async function basicInterations() {
  ...
  var our_contract = await new web3.eth.Contract(
    our_contract_abi,
  ...
  console.log(
    `Our Contract address in other way:  ${our_contract.options.address}`
  );

  // Now our contract abi
  console.log(
    "Our contract abi: " + JSON.stringify(our_contract.options.jsonInterface)
  );
  ...
  console.log(
    `Total Supply of Our Contract address ${our_contract._address}:  ${totalSupply}`
  );

  // Now let's see our contract public variable name
  var name = await our_contract.methods.name().call();
  console.log(
    `Public variable name of our Contract address ${our_contract._address}:  ${name}`
  );

  // Now let's see our contract public variable symbol
  var symbol = await our_contract.methods.symbol().call();
  console.log(
    `Public variable symbol of our Contract address ${our_contract._address}:  ${symbol}`
  );
}
